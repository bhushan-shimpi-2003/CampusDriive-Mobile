...
export default function CoordinatorAnnouncementFeedScreen() {
  ...
  const filteredAnnouncements = useMemo(() => {
    ...
    // Filter to show only announcements sent by coordinators or system
    anns = anns.filter(a => a.authorRole === 'coordinator' || a.authorRole === 'system');
    return anns;
  }, [category]);

  const handlePress = (ann: typeof mockAnnouncements[0]) => {
    if (!ann.relatedId) return;
    if (ann.category === 'communities') {
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            description="You haven't sent any announcements to your assigned communities."
