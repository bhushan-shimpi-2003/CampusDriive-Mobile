...
const EVENT_TYPES: { label: string; value: CalendarEventType | 'all' }[] = [
  ...
  { label: 'Drive Days', value: 'drive' },
  { label: 'Interviews', value: 'interview' },
  { label: 'Verification Deadlines', value: 'deadline' },
];
...
export default function CoordinatorCalendarScreen() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    // Coordinators mainly care about drives, interviews, and deadlines.
    evs = evs.filter(e => ['drive', 'interview', 'deadline'].includes(e.type));
  ...
  const handleEventPress = (event: typeof mockCalendarEvents[0]) => {
    if (event.relatedId && (event.type === 'drive' || event.type === 'interview' || event.type === 'deadline')) {
      navigation.navigate('CoordinatorDriveDetails', { driveId: event.relatedId });
    }
  };

  return (
    <SafeAreaView style={globalStyles.safeContainer}>
      <AppHeader title="Task Calendar" onBack={() => navigation.goBack()} />
      ...
      {filteredEvents.length === 0 ? (
        <EmptyState
          ...
          description="There are no upcoming assigned drives or verifications."
